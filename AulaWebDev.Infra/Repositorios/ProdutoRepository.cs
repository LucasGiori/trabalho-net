...
namespace AulaWebDev.Infra.Repositorios
{
    public class ProdutoRepository : IProdutoRepository
    {
        ...
        {
            return await _dbContext.Produtos.Include(x => x.Category).FirstOrDefaultAsync(x => x.Codigo == codigo);
        }

        public async Task<Produto?> ObterPorIdAsync(Guid id)
        {
            return await _dbContext.Produtos.Include(x => x.Category).FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<ICollection<Produto>> ObterTodosAsync()
        {
            return await _dbContext.Produtos.Include(x => x.Category).ToListAsync();
        }
    }
}
